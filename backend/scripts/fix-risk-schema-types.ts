#!/usr/bin/env bun
/**
 * Fix Risk Schema - Change user_id and tenant_id from UUID to TEXT
 * This matches the auth.users and tenants schemas which use TEXT
 */
...
async function fixSchema() {
  try {
    console.log('📊 Fixing Risk module table types...\n');

    // Drop and recreate tables with correct types
    console.log('  🗑️  Dropping existing tables...');
    await db.execute(sql`DROP TABLE IF EXISTS risk_alerts CASCADE;`);
    await db.execute(sql`DROP TABLE IF EXISTS risk_limits CASCADE;`);
    await db.execute(sql`DROP TABLE IF EXISTS risk_metrics CASCADE;`);
    await db.execute(sql`DROP TABLE IF EXISTS risk_profiles CASCADE;`);

    // 1. Risk Profiles Table with TEXT for user_id and tenant_id
    console.log('  ✅ Creating risk_profiles table (TEXT IDs)...');
    ...
    await db.execute(sql`
      CREATE INDEX risk_profiles_user_idx ON risk_profiles(user_id);
    `);
    await db.execute(sql`
      CREATE INDEX risk_profiles_tenant_idx ON risk_profiles(tenant_id);
    `);

    // 2. Risk Limits Table
    console.log('  ✅ Creating risk_limits table (TEXT IDs)...');
    await db.execute(sql`
    ...
    await db.execute(sql`
      CREATE INDEX risk_limits_user_idx ON risk_limits(user_id);
    `);
    await db.execute(sql`
      CREATE INDEX risk_limits_profile_idx ON risk_limits(profile_id);
    `);
    await db.execute(sql`
      CREATE INDEX risk_limits_type_idx ON risk_limits(limit_type);
    `);
    await db.execute(sql`
      CREATE INDEX risk_limits_enabled_idx ON risk_limits(enabled);
    `);

    // 3. Risk Metrics Table
    console.log('  ✅ Creating risk_metrics table (TEXT IDs)...');
    await db.execute(sql`
    ...
    await db.execute(sql`
      CREATE INDEX risk_metrics_user_idx ON risk_metrics(user_id);
    `);
    await db.execute(sql`
      CREATE INDEX risk_metrics_tenant_idx ON risk_metrics(tenant_id);
    `);
    await db.execute(sql`
      CREATE INDEX risk_metrics_calculated_at_idx ON risk_metrics(calculated_at);
    `);
    await db.execute(sql`
      CREATE INDEX risk_metrics_snapshot_idx ON risk_metrics(snapshot_date);
    `);

    // 4. Risk Alerts Table
    console.log('  ✅ Creating risk_alerts table (TEXT IDs)...');
    await db.execute(sql`
    ...
    await db.execute(sql`
      CREATE INDEX risk_alerts_user_idx ON risk_alerts(user_id);
    `);
    await db.execute(sql`
      CREATE INDEX risk_alerts_tenant_idx ON risk_alerts(tenant_id);
    `);
    await db.execute(sql`
      CREATE INDEX risk_alerts_type_idx ON risk_alerts(alert_type);
    `);
    await db.execute(sql`
    ...
    console.log('   ✅ user_id and tenant_id are now TEXT (matching auth.users and tenants)');
